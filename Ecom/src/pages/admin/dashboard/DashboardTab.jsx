...
function DashboardTab() {
    ...
    const { mode, product, edithandle, deleteProduct, order, user, approveProduct } = context
                    ...
                    <Tabs defaultIndex={0} className=" " >
                                ...
                                <div className="relative overflow-x-auto ">
                                    <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400  ">
                                        ...
                                        {product.map((item, index) => {
                                            const { title, price, imageUrl, category, date, uid, ownerName, ownerContact } = item;
                                            return (
                                                <tbody className=''>
                                                        ...
                                                        <td className="px-6 py-4">
                                                            <div className=" flex gap-2">
                                                                <div className=" flex gap-2 cursor-pointer text-black " style={{ color: mode === 'dark' ? 'white' : '' }}>
                                                                    ...
                                                                    <div onClick={() => approveProduct(item)}  >
                                                                        {/* approve svg icon */}
                                                                        {!item.approved && <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={7} stroke="green" className="w-6 h-6">
                                                                            <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                                                                        </svg>}
                                                                        {item.approved && <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={7} stroke="red" className="w-6 h-6">
                        ...
                        <TabPanel>
                            <div className="relative overflow-x-auto mb-16">
                                ...
                                <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                                {/* Table Head - displayed only once */}
                                ...
                                <tbody>
                                    ...
                                    <>
                                        {/* First row for the order, grouped with the cart items */}
                                        <tr key={`order-${orderIndex}`} className="bg-gray-50 border-b dark:border-gray-700" style={{ backgroundColor: mode === 'dark' ? 'rgb(46 49 55)' : '', color: mode === 'dark' ? 'white' : '' }}>
                                        <td className="px-6 py-4 text-black font-bold" colSpan={10} style={{ color: mode === 'dark' ? 'white' : '' }}>
                                            Order {orderIndex + 1}: <span className="font-medium">Payment Id: {allorder.orderId}</span>, Date: {allorder.date}, Customer: {allorder.name} ({allorder.phoneNumber}, {allorder.email})
                                        </td>
                                        </tr>

                                        {/* Loop through each item in the cartItems for the current order */}
                                        ...
                                        return (
                                            ...
                                            <td className="px-6 py-4 text-black" style={{ color: mode === 'dark' ? 'white' : '' }}>{allorder.orderId}</td>
                                            <th scope="row" className="px-6 py-4 font-medium text-black whitespace-nowrap">
                                                <img className="w-16" src={imageUrl} alt="img" />
                                            </th>
                                            ...
                                            <td className="px-6 py-4 text-black" style={{ color: mode === 'dark' ? 'white' : '' }}>{allorder.name}</td>
                                            <td className="px-6 py-4 text-black" style={{ color: mode === 'dark' ? 'white' : '' }}>{allorder.phoneNumber}</td>
                        ...
                        <TabPanel>
                            <div className="relative overflow-x-auto mb-10">
                                ...
                                <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                                    ...
                                    {user.map((item, index) => {
                                        const {name,uid,email,time, date} = item;
                                        return (
                                            <tbody>
                                                <tr className="bg-gray-50 border-b  dark:border-gray-700" style={{ backgroundColor: mode === 'dark' ? 'rgb(46 49 55)' : '', color: mode === 'dark' ? 'white' : '', }} >
                                                    <td className="px-6 py-4 text-black " style={{ color: mode === 'dark' ? 'white' : '' }}>
